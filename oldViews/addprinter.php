...
if (isset($_POST['submit'])){
    $printer = Walltwisters\lib\model\Printer::create($_POST['printername'], $_POST['email'], $_POST['telephonenumber'], $_POST['contactperson'], $_POST['country'],$user->id);
    $printerid = $printerservice->addPrinter($printer);
    $items = [];
    $itemPrices = [];
    for ($i = 0; $i < count($_POST['paper']); $i++){
        $item = Walltwisters\lib\model\PrinterItem::create($_POST['paper'][$i], 
                               $_POST['technique'][$i],
                               $_POST['size'][$i]);
        $itemPrice = Walltwisters\lib\model\ItemPrice::create($_POST['paperprice'][$i],
